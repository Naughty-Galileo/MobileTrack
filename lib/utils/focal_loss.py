...
# class QualityFocalLoss(nn.Module, ABC):
...
#         positive_loss = w * torch.pow((1 - pt), self.gamma) * torch.log(pt) * positive_index * sample_weight
#         negative_loss = w * torch.pow(pt, self.gamma) * torch.log(1 - pt) * negative_index * sample_weight

#         num_positive = torch.sum(positive_index.float() * sample_weight)
#         positive_loss = positive_loss.sum()
#         negative_loss = negative_loss.sum()
