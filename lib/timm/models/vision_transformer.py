...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        match_size = True if x.size(-1) == self.img_size else False
        if not match_size:
            extra_tokens = self.pos_embed[:, :self.num_tokens]
            pos_tokens = self.pos_embed[:, self.num_tokens:]
            ori_size = self.img_size // self.patch_size
            new_size = x.size(-1) // self.patch_size
            pos_tokens = pos_tokens.reshape(-1, ori_size, ori_size, self.num_features).permute(0, 3, 1, 2)
            ...
            feat_sz = self.img_size // self.patch_size
        x = self.patch_embed(x)  # (bs, L, c)
        ...
        if self.dist_token is None:
            ...
            opt = x[:, 2:]
        return opt.transpose(-1, -2).view(opt.size(0), self.num_features, feat_sz, feat_sz)

    # def forward_backbone_features(self, x):
    #     """2021.04.12 Take the backbone feature"""
